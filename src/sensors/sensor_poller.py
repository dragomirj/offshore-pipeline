...
from src.sensors.constants import SENSOR_POLLER_MAX_ERRORS
        ...
        try:
            while True:
                try:
                    ...
                    self._error_count += 1
                    logger.warning(
                        "Sensor Poller failure - sensor=%s count=%d/%d error=%s",
                    ...
                    if self._error_count >= self._max_errors:
                        raise RuntimeError(
                            f"Readable failed {self._max_errors} consecutive times: {e}"
                        ) from e
